        ...
        <header className={`flex flex-wrap md:justify-start md:flex-nowrap z-50 w-full fixed top-0 left-0 right-0 border-b border-white/10 shadow-sm transition-all duration-300 ${isScrolled ? 'bg-black/40 backdrop-blur-sm' : 'bg-transparent backdrop-blur-sm'}`}>
            <nav className="relative max-w-7xl w-full md:flex md:items-center md:justify-between md:gap-3 mx-auto px-4 sm:px-6 lg:px-8 py-3">
                <div className="flex items-center justify-between">
                    <Link className={`flex-none font-semibold text-xl ${isScrolled ? 'text-slate-50' : 'text-slate-50'} focus:outline-none focus:opacity-80`} to="/" aria-label="Brand">
                ...
                <div id="hs-base-header" className={`hs-collapse overflow-hidden transition-all duration-300 basis-full grow ${isMenuOpen ? 'block' : 'hidden'} md:block`} aria-labelledby="hs-base-header-collapse">
                    <div className="overflow-hidden overflow-y-auto max-h-[75vh] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300">
                        <div className="py-2 md:py-0 flex flex-col md:flex-row md:items-center md:justify-end gap-0.5 md:gap-1">
                            <Link className={`p-2 flex items-center text-sm ${isScrolled ? 'text-slate-50' : 'text-slate-50'} focus:outline-none`} to="/" aria-current="page">
                            ...
                            <Link className={`p-2 flex items-center text-sm ${isScrolled ? 'text-slate-50' : 'text-slate-50'} hover:text-slate-50 focus:outline-none`} to="/search-lands">
                                <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    <path d="M12 12h.01" />
                            ...
                            <Link to='/pricing-plans' className={`p-2 flex items-center text-sm ${isScrolled ? 'text-slate-50' : 'text-slate-50'} hover:text-slate-50 focus:outline-none`}>
                                <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            ...
                            <Link className={`p-2 flex items-center text-sm ${isScrolled ? 'text-slate-50' : 'text-slate-50'} hover:text-slate-50 focus:outline-none`} to="/contact-us">
                                <svg className="shrink-0 size-4 me-3 md:me-2 block md:hidden" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
